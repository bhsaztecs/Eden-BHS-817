#pragma once
bool isCreate = false;//ADD FUNCTIONALITY

#include <D:/kipr/wombat.h>//pc
#include <iostream>
#include <string>
#include <chrono>
#include <vector>
...
//__PRETTY_FUNCTION__ instead of __func__, only on GCC
#define DLOG if(Debugging) {\
	...
	std::cout << __func__ << " (" << __LINE__ << ") Time since start: " << PrettyTime(elapsedtime.count()) << std::endl;\
}
...
class misc;//other stuff i couldnt lable
class dgtl;//digital sensors
class nlg;//analog sensors
class accel;//accelerometer control
class gyro;//gyrometer control
class mag;//magnetometer control NOT YET IMPLEMENTED
...
// guess until youre right... supposed to be (axle length / wheel length ) +1 but that gives like 3.6 or sumn and that doesnt work so whatever... maybe my unit conversions are wrong somewhere idk man
...
public:
	...
	/*
	P2D operator-(const worldSpace& other) {
		return P2D(this->X - other.X, this->Y - other.Y);
	}
	P2D operator+(const worldSpace& other) {
		return P2D(this->X + other.X, this->Y + other.Y);
	}
	void operator=(const worldSpace& other) {
		this->X = other.X; this->Y = other.Y;
	}
...
public:
	...
	static void Start(bool);//initializes variables
...
class motors {
public:
	motors(int leftport, int rightport, float leftmult = 1, float rightmult = 1) {
...
int DTTA(float degrees) {
	DLOG
		return (0.09 / degrees) + 57.15;
}

float TTDA(int tics) {
	DLOG
		return (0.09 * tics) - 57.15;
}
