...
void Motors::Clear() const { BKND::motors::ClearMotorRotations(m_pass); }
void Motors::Velocity() const { BKND::motors::Velocity(m_pass); }
...
void Motors::Brake() const { BKND::motors::Brake(m_pass); }

Servos::Servos(int p_port, BKND::P2D p_min, BKND::P2D p_max) {}
void Servos::Set(float p_angle) const { BKND::servos::Set(m_Port, p_angle); }
void Servos::Change(float p_angle) const {
  BKND::servos::Change(m_Port, p_angle);
}
void Servos::GoTo(float p_angle, float p_time) const {
  BKND::servos::Move(m_Port, p_angle, p_time);
}
float Servos::Angle() const { return BKND::TTDA(get_servo_position(m_Port)); }
